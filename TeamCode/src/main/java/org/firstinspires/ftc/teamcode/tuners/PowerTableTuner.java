...
public class PowerTableTuner extends OpMode {
    ...
    public void loop() {
        ...
        else if (x2state) {robotOuttake.run(poses.getOptimalShooterPowerPercentage(follower.getPose(), true));}
        ...
        if (dpu2wP && !offToggle && !b2state) {robotOuttake.tuneActivePreset(0.005);}
        else if (dpd2wP && !offToggle) {robotOuttake.tuneActivePreset(-0.005);}
        ...
        log("Outtake Preset Speed", robotOuttake.closeShotPower);
        ...
        log("Angle (deg)", Math.toDegrees(currentPose.getHeading()));
        log("Distance from Goal", follower.getPose().distanceFrom(poses.goalPose));
